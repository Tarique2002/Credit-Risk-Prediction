...
import { ShieldCheck, Lock, Mail, RefreshCw, ChevronLeft } from "lucide-react";
...
export const Login: React.FC<LoginProps> = ({ isAdminEntry = false }) => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 relative overflow-hidden bg-[#030712]">
      ...
      <div className="absolute bottom-1/4 right-1/4 w-[300px] h-[300px] bg-cyan-500/5 rounded-full blur-[80px] pointer-events-none -z-10" />
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            {error && (
              <motion.div
                ...
                className="bg-red-500/10 border border-red-500/20 text-red-200 p-3.5 rounded-2xl text-[10px] text-center font-mono leading-relaxed"
              >
                {error}
              </motion.div>
            )}
            ...
            <motion.div variants={itemVariants} className="space-y-2">
              <div className="flex justify-between items-center">
                ...
                <Magnetic strength={0.2} scale={1.03}>
                  <Link
                    to="/forgot-password"
                    className="text-[10px] text-cyan-400 hover:text-cyan-300 font-medium transition-colors font-mono uppercase tracking-wider"
                  >
              ...
              />
              {errors.password && (
                <p className="text-[10px] font-mono text-red-400 mt-1">{errors.password.message}</p>
              )}
            </motion.div>
